...
from fairseq.models.transformer import DEFAULT_MIN_PARAMS_TO_WRAP, TransformerEncoder_ST
...
@register_model("roberta_st")
class RobertaModel_ST(FairseqEncoderModel):
    @classmethod
    def hub_models(cls):
        return {
    ...
    def add_args(parser):
        ...
        ## new args for controlling the learnable masks
        parser.add_argument(
            "--prune_rate",
            type=float,
            default=0.9,
    ...
    def build_model(cls, args, task):
        ...
        encoder = RobertaEncoder_ST(args, task.source_dictionary)
        ...
        for n, m in encoder.named_modules():
            if hasattr(m, "in_proj_weight") and m.in_proj_weight is not None:
                print(f"==> No gradient to {n}.in_proj_weight")
                m.in_proj_weight.requires_grad = False
                if m.in_proj_weight.grad is not None:
                    print(f"==> Setting gradient of {n}.in_proj_weight to None")
                    m.in_proj_weight.grad = None
                
                if hasattr(m, "in_proj_bias") and m.in_proj_bias is not None:
                    print(f"==> No gradient to {n}.in_proj_bias")
                    m.in_proj_bias.requires_grad = False

                    if m.in_proj_bias.grad is not None:
                        print(f"==> Setting gradient of {n}.in_proj_bias to None")
                        m.in_proj_bias.grad = None

            if hasattr(m, "weight") and m.weight is not None:
                if 'classification_heads' in n:  ## For the final classification head, do not stop their gradients
                    continue
        ...
        for n, m in encoder.named_modules():
            if hasattr(m, "set_prune_rate"):
                if args.fix_attn:
                    if 'self_attn' in n: # if 'k_proj' in n or 'v_proj' in n or 'q_proj' in n or 'out_proj' in n:
                        prune_rate = 1.0
                        m.use_subset = False
                    else:
        ...
        if args.init_score != 'kaiming_uniform':
            for n, m in encoder.named_modules():
                if hasattr(m, "init_score"):
                    if args.init_score == 'weight_magnitude_with_scale':
                        m.init_score(init_method=args.init_score, scale=args.init_score_scale)
                    else:
                        m.init_score(init_method=args.init_score)

                    print(f"==> Setting init score of {n} with:", args.init_score)
        
        if 'fix_mask' in args and args.fix_mask:
            for n, m in encoder.named_modules():
                if hasattr(m, "disable_score_grad"):
                    m.disable_score_grad()
                    print(f'Disable the gradient in the score of {n}')

        elif 'fix_mask_before' in args and args.fix_mask_before > 0:
            for n, m in encoder.named_modules():
                layer_id = re.findall('\.(\d+)\.', n)
                if len(layer_id) > 0 and int(layer_id[0]) < args.fix_mask_before and hasattr(m, "disable_score_grad"):
                    m.disable_score_grad()
                    print(f'Disable the gradient in the score of {n}')
...
class RobertaEncoder_ST(FairseqEncoder):
    """RoBERTa encoder."""
    ...
    def build_encoder(self, args, dictionary, embed_tokens):
        encoder = TransformerEncoder_ST(args, dictionary, embed_tokens)
...
@register_model_architecture("roberta_st", "roberta_st")
...
@register_model_architecture("roberta_st", "roberta_prenorm_st")
...
@register_model_architecture("roberta_st", "roberta_base_st")
def roberta_base_architecture(args):
    base_architecture(args)


@register_model_architecture("roberta_st", "roberta_large_st")
...
@register_model_architecture("roberta_st", "xlm_st")
