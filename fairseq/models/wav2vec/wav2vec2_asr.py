...
class Wav2Vec2CtcConfig(Wav2Vec2AsrConfig):
    ...
    ft_fc_only: bool = field(
        default=False,
        metadata={
            "help": "Only finetuning the fc layers"
        },
    )

    ft_attn_only: bool = field(
        default=False,
        metadata={
            "help": "Only finetuning the self-attn layers"
        },
    )
    ...
    use_adapter: bool = field(
        default=False,
        metadata={
            "help": "Use adapters for finetuning"
        },
    )
...
class Wav2VecCtc(BaseFairseqModel):
    ...
    def build_model(cls, cfg: Wav2Vec2CtcConfig, task: FairseqTask):
        ...
        for n, m in w2v_encoder.named_modules():
            ...
            if cfg.ft_fc_only:
                fix_flag = 'k_proj' in n or 'v_proj' in n or 'q_proj' in n or 'out_proj' in n
            
            if cfg.ft_attn_only:
                fix_flag = 'fc1' in n or 'fc2' in n
            
            if cfg.use_adapter:
                fix_flag = 'adapter' not in n and n != 'proj'

            if fix_flag and hasattr(m, "weight") and m.weight is not None:
        ...
        for name, param in w2v_encoder.named_parameters():
...
class Wav2VecEncoder(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrConfig, output_size=None):
        ...
        if cfg.use_adapter:
            w2v_args.model['_name'] = w2v_args.model['_name'] + '_adapter'
            
            with open_dict(w2v_args):
                w2v_args.model.adapter_type = cfg.adapter_type
