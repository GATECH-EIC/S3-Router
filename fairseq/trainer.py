...
class Trainer(object):
    ...
    def state_dict(self):
        ...
        if not self.cfg.checkpoint.no_save_optimizer_state:
            if hasattr(self, '_gathered_optim_state') and self._gathered_optim_state is not None:
    ...
    def save_checkpoint(self, filename, extra_state, model_only=False):
        """Save all training state in a checkpoint file."""
        logger.info(f"Saving checkpoint to {filename}")
        # call state_dict on all ranks in case it needs internal communication
        if model_only:
            state_dict = utils.move_to_cpu(self.model_state_dict())
        else:
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                if "w2v_encoder.proj.weight" in state["model"]:
                    if state["model"]["w2v_encoder.proj.weight"].shape != self.model.state_dict()["w2v_encoder.proj.weight"].shape:
                        state["model"].pop("w2v_encoder.proj.weight")
                        state["model"].pop("w2v_encoder.proj.bias")

                if 'load_mask_only' in cfg_model and cfg_model.load_mask_only:
                    import re

                    key_list = deepcopy(list(state["model"].keys()))
                    for key in key_list:
                        if 'scores' not in key:
                            state["model"].pop(key)
                    
                        elif 'load_mask_before' in cfg_model and cfg_model.load_mask_before:
                            layer_id = re.findall('\.(\d+)\.', key)
                            if len(layer_id) > 0 and int(layer_id[0]) > cfg_model.load_mask_before:
                                state["model"].pop(key)
                    
                    # print(state["model"].keys())

                    missing_keys, unexpected_keys = self.model.load_state_dict(state["model"], strict=False, model_cfg=self.cfg.model)
                    ...
                    for missing_key in missing_keys:
                        assert 'scores' in missing_key or 'w2v_encoder.proj' in missing_key or 'thres' in missing_key, f'missing {missing_key}'
